 ...
 const filterData = ()=>{
  if(departure&&destination&&date)
  {
    const filter = flightsData?.data?.result?.filter((item:any)=>{
      ...
      const formattedDate = `${inputDateTime.getDate().toString().padStart(2, '0')}-${(inputDateTime.getMonth() + 1).toString().padStart(2, '0')}-${inputDateTime.getFullYear()}`;
      return formattedDate==date&&(item?.displayData?.source?.airport?.cityName==departure&&item?.displayData?.destination?.airport?.cityName==destination)
    })
  ...
  {
    const filter = flightsData?.data?.result?.filter((item:any)=>{
      return (item?.displayData?.source?.airport?.cityName==departure&&item?.displayData?.destination?.airport?.cityName==destination)
    })
    //console.log('filter date',filter)
    setFlights(filter)
  }
  ...
  return (
    ...
    <View style={styles.headerContainer}>
      ...
      <View style={styles.destinationContainer}>
        <View>
            {/* departure */}
            <Text style={styles.destinationTitle}>{departure?departure:'Delhi'}</Text>
        ...
        <View>
            {/* Destination */}
            <Text style={styles.destinationTitle}>{destination?destination:'Mumbai'}</Text>
      ...
      <View style={styles.scrollDatesContainer}>
       ...
       showsHorizontalScrollIndicator={false}
        style={styles.scrollStyle} contentContainerStyle={styles.scrollContentStyle}>
        {Dates.map((item,index) => {
          ...
          return (
            ...
            >
            <View key={index} style={[styles.dates,{
        ...
        return (
          ...
          style={{
            ...
            borderWidth:1,
              ...
              elevation:1,
              shadowColor: "white",
              backgroundColor:flightData?.id==index?'#e6ffe6':'transparent'
          }}
          onPress={()=>{
          ...
          >
            ...
            <View style={{
              ...
              <Text style={styles.destinationTitle}>{item?.displayData?.source?.airport?.airportName}</Text>
            ...
            <View style={{
              ...
              <Text style={[styles.destination,{textAlign:"right"}]}>{item?.displayData?.source?.airport?.cityName}</Text>
              <View style={[styles.logoContainer,{
                marginVertical:0,
              }]}><Image source={Jahaaj} style={styles.icon} resizeMode='contain' /></View>
              <Text style={[styles.destination,{
                textAlign:"left"
              }]}>{item?.displayData?.destination?.airport?.cityName}</Text>
            </View>
            ...
            <View style={{
              ...
              <Text>{formatDate(item?.displayData?.destination?.arrTime)}</Text>
            </View>
          </TouchableOpacity>
        )
      })}
